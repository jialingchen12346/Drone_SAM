...
- 决策：执行阶段性归档并切换到 SOTA 对标阶段。
- 背景：前一阶段已完成口径纠偏与旧路线复盘，需要冻结事实基线后再进入新一轮模型选型。
- 影响：
  - 新增阶段快照：`research_workspace/artifacts/stage_archives/2026-03-25_pre_sota/`（含 `MANIFEST.md`）。
  - 新增 SOTA 快照：`research_workspace/notes/SOTA_FMB_SNAPSHOT_2026-03-25.md`（含 `2503.02581` 重点分析与可核对标表）。
  - 后续外部对比统一要求“原始口径 + strict 口径”双栏呈现，避免再次出现不可比结论。

## 2026-03-30

- 决策：研究主线从“FMB 全监督结构卷分”切换为“标签效率导向（半监督 + 弱监督）”。
- 背景：FMB 全监督方向已有高分工作，继续堆模块的边际收益与可发表性下降；当前更需要可迁移、可复现、符合趋势的问题定义。
- 影响：
  - `research_workspace/plans/current_plan.yaml` 已重写为 label-efficiency 主线。
  - 新增执行文档 `research_workspace/plans/LABEL_EFFICIENT_RS_PLAN_2026-03-30.md`。
  - 后续实验主指标从“单点 mIoU”扩展为“性能 + 标注成本 + 鲁棒性 + 复杂度”。

## 2026-03-30

- 决策：数据集策略调整为“FMB 短期主实验 + 遥感数据集中期补证据”。
- 背景：完全脱离 FMB 会损失既有工程资产与短周期可执行性；仅做 FMB 又不足以支撑新叙事。
- 影响：
  - 短期保持 FMB 作为半监督/弱监督主战场。
  - 中期至少补齐一个遥感公开集（LoveDA/ISPRS/UAVid）可复现实验。
  - 论文对外比较将强调跨场景趋势一致性，而非单榜极值。

## 2026-03-30

- 决策：投稿节奏由“PRCV 优先冲线”调整为“BMVC 主投稿 + ACCV 备投增强”。
- 背景：主线切换后需要协议重建与方法实现，PRCV 截止窗口不足以支撑完整证据链。
- 影响：
  - `2026-03-30 ~ 2026-05-22` 目标为形成 BMVC 可投稿版本。
  - `2026-05-23 ~ 2026-07-05` 聚焦扩展实验与 ACCV 增强稿。
  - PRCV 若提交，仅可作为阶段性短稿，不作为主线门槛。
